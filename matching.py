...
def do_matching(image, template):
    ...
    some_image = np.divide(finding_corelation, np.absolute(finding_corelation))
    some_image_real = np.real(some_image)
    maxVal = np.max(some_image_real)
    minVal = np.min(some_image_real)
    some_image_real_display = np.copy(some_image_real)
    # print real_component.shape
    for i in range(some_image_real.shape[0]):
        for j in range(some_image_real.shape[1]):
            some_image_real_display[i][j] = ((some_image_real[i][j] - minVal) * 255) / (maxVal - minVal)
    real_comp_display = some_image_real_display.astype(np.uint8)

    inverse_transform = np.fft.ifft2(
        some_image)  # take the inverse transform
    real_component = np.real(inverse_transform)



    cv2.imshow("adaasda",some_image_real_display)
